...
internal class DKBundleHelper {
    ...
    static func bundle(named name: String) -> Bundle? {
        let frameworkBundle = Bundle(for: DKBundleHelper.self)
    ...
    static func color(named name: String,
        ...
        if let imageBundle = bundle {
            ...
            return UIColor(named: name, in: DKBundleHelper.bundle(named: bundleName), compatibleWith: traitCollection)
        } else {
            return nil
        }
    }
    ...
    static func image(named name: String,
        ...
        if let imageBundle = bundle {
            ...
            return UIImage(named: name, in: DKBundleHelper.bundle(named: bundleName), compatibleWith: traitCollection)
        } else {
            return nil
        }
    }
    ...
    static func image(wrappedImage: WrappedBundleImage,
                      in bundle: Bundle? = nil,
                      bundleName: String? = nil,
                      compatibleWith traitCollection: UITraitCollection? = nil) -> UIImage? {
        return DKBundleHelper.image(named: wrappedImage.resourceName, in: bundle, bundleName: bundleName, compatibleWith: traitCollection)
    }
    ...
    static func nib(named name: String, in bundle: Bundle? = nil, bundleName: String? = nil) -> UINib {
        if let nibBundle = bundle {
            return UINib(nibName: name, bundle: nibBundle)
        } else if let bundleName = bundleName {
            return UINib(nibName: name, bundle: DKBundleHelper.bundle(named: bundleName))
        } else {
            return UINib()
        }
    }
    ...
    static func storyboard(named name: String, in bundle: Bundle? = nil, bundleName: String? = nil) -> UIStoryboard {
        if let storyboardBundle = bundle {
            return UIStoryboard(name: name, bundle: storyboardBundle)
        } else if let bundleName = bundleName {
            return UIStoryboard(name: name, bundle: DKBundleHelper.bundle(named: bundleName))
        } else {
            return UIStoryboard()
        }
    }

    /// Utility function to check if current App is UHC.
    @available(*, deprecated, message: """
    DKBundleHelper is a generic helper class and shouldn't hold isUHC should be moved to a more appropriate location for the modules that need it
    """)
    static var isUHC: Bool? {
        return DKBundleHelper.getConfigurableValueForKey(key: "isUHC", fileName: "AppConfig", fileType: "plist") as? Bool
    }
    ...
    static func getConfigurableValueForKey(key: String, fileName: String, fileType: String) -> Any? {
        return DKBundleHelper.getGlobalValueForKey(key: key, bundle: Bundle.main, fileName: fileName, fileType: fileType)
    }
    ...
    static func getGlobalValueForKey(key: String, bundle: Bundle, fileName: String, fileType: String) -> Any? {
        ...
        if let plistPath = bundle.path(forResource: fileName, ofType: fileType),
            ...
            do {//convert the data to a dictionary and handle errors
                if let plistData = try PropertyListSerialization.propertyList(from: plistXML,
